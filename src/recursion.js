...
var rMap = function(array, callback) {
    ...
    var curVal = callback(array[0]);
    return [curVal].concat(rMap(array.slice(1), callback));
};
...
var countKeysInObj = function(obj, key) {
    ...
    var keyCount = keyArr.reduce(function(accum, keyEle) {
        if (keyEle === key) {
            return accum + 1;
        } else {
            return accum;
    ...
    valArr.forEach(function(val) {
        if (typeof val === 'object') {
            keyCount += countKeysInObj(val, key);
        }
    });
...
var countValuesInObj = function(obj, value) {
    ...
    valArr.forEach(function(val) {
        if (typeof val === 'object'){
            valCount += countValuesInObj(val, value);
        } else {
            if (val === value) {
                valCount ++;
            }
...
var fibonacci = function(n) {
    ...
    if (n > 1) {
        ...
        var curEle = array[array.length - 1] + array[array.length -2];
        return array.concat([curEle]);
    }
...
var capitalizeWords = function(array) {
    ...
    var curEle = array[0].toUpperCase();
    return [curEle].concat(capitalizeWords(array.slice(1)));
};
...
var capitalizeFirst = function(array) {
    ...
    var curEle = array[0][0].toUpperCase() + array[0].slice(1);
    return [curEle].concat(capitalizeFirst(array.slice(1)));
};
...
var numToText = function(str) {
    ...
    if (Number.isNaN(Number.parseInt(str[0]))) {
        ...
        return lookUp[Number.parseInt(str[0])].concat(numToText(str.slice(1)));
    }
};
...
var tagCount = function(tag, node) {
    ...
    if (node.firstElementChild !== null) {
        curCount += tagCount(tag, node.firstElementChild);
    }

    if (node.nextElementSibling !== null) {
        curCount += tagCount(tag, node.nextElementSibling);
    }
